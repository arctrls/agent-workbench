...
- YAML이 깨지지 않도록, 아래 frontmatter 문자열 필드는 **항상 큰따옴표(`"`)로 감싸세요**:
  - `id`, `aliases`, `author`, `created_at`, `source`, `article_id`
- 제목/별칭에 `:`(콜론), `#`, `[ ]`, `{ }`, `'`, `"` 등이 포함될 수 있으므로 **무조건 quoted scalar** 사용
- 문서 저장 직전에 frontmatter YAML 파싱 검증을 반드시 수행하고, 실패 시 저장하지 말고 수정 후 재검증
...
6. 파일 저장 전 YAML frontmatter를 검증:
